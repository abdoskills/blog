...
export const metadata = {
  title: "Timeline 0: Linux Filesystem MAC Timeline Analysis (Timestomped Outlier) | PicoCTF 2026",
  description: "Forensic analysis of a raw ext4 partition image using The Sleuth Kit (TSK). Reconstructing chronological MAC timelines with fls and mactime to identify a timestomped 1985 temporal outlier.",
};

export default function PicoCTFTimeline0Writeup() {
  const flsCmd = `fls -m / -r ./partition4.img | Out-File -Encoding ascii body.txt`;
  const mactimeCmd = `mactime -b body.txt -d -z UTC | Out-File -Encoding ascii timeline.csv`;
  const inspectCmd = `Get-Content timeline.csv | Select-Object -First 5`;
  const icatCmd = `icat.exe ./partition4.img 4945`;
  const decodeCmd = `[System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String("NzFtMzExbjNfMHU3MTEzcl9oM3JfNDNhMmU3YWYK"))`;
  const flagText = `picoCTF{71m311n3_0u7113r_h3r_43a2e7af}`;
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 pb-24">
        ...
        <header className="mb-10 flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold mb-6 text-white font-[family-name:var(--font-share-tech)] leading-tight break-words">
            Timeline 0: Linux Filesystem MAC Timeline Analysis
          </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 w-full text-left font-mono text-xs">
            ...
            <div className="bg-[#0e0e13]/90 border border-zinc-800 p-3 sm:p-4 rounded-xl min-w-0">
              ...
              <span className="text-amber-400 font-bold truncate block">4945 (/bin/bcab)</span>
            </div>
            <div className="bg-[#0e0e13]/90 border border-zinc-800 p-3 sm:p-4 rounded-xl min-w-0">
              <span className="text-zinc-500 block mb-1 text-[11px]">FLAG</span>
              <span className="text-emerald-400 font-bold truncate block">picoCTF&#123;71m311...&#125;</span>
            </div>
          </div>
        </header>
        ...
        <div className="relative w-full aspect-video rounded-2xl overflow-hidden border border-zinc-800/80 mb-12 shadow-2xl group bg-black">
          <Image
            src="/images/pico_timeline_0.jpg"
            alt="Linux Filesystem MAC Timeline Outlier Forensics"
            fill
            priority
          ...
          <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center text-xs font-mono text-zinc-400">
            <span>FIGURE 1: Forensic Workbench &amp; Filesystem Inode Triage</span>
            <span className="text-cyan-400 font-bold">The Sleuth Kit • Timestomping • Outlier</span>
          </div>
        </div>
        ...
        <div className="bg-[#0b1418]/90 border border-cyan-500/30 rounded-2xl p-4 sm:p-6 shadow-xl relative overflow-hidden mb-12 backdrop-blur-md">
          <div className="space-y-3">
            ...
            <p className="text-zinc-300 text-sm md:text-base leading-relaxed font-sans">
              We are tasked with investigating a raw ext4 partition image (<code className="text-cyan-300 bg-black/40 px-1 py-0.5 rounded font-mono text-xs">partition4.img</code>). In this scenario, the adversary did not leave files in the latest session; instead, they attempted to hide by backdating file timestamps to an extreme date in the past. Using <strong>The Sleuth Kit</strong>, we extract metadata into a standardized bodyfile, compile a chronological MAC timeline, identify the <strong>1985 temporal outlier</strong>, and carve out the hidden payload directly from Inode 4945.
            </p>
          </div>
        </div>
        ...
        {/* Section 3: Detecting Temporal Outlier */}
        <section className="mb-12 space-y-6">
          <h2 className="text-xl sm:text-2xl font-bold text-white border-b border-zinc-800 pb-3 flex items-center gap-3 font-[family-name:var(--font-share-tech)] break-words">
            <span className="text-cyan-400">03.</span> Detecting the Timestomped 1985 Outlier
          </h2>
          <p className="text-zinc-300 leading-relaxed text-sm sm:text-base">
            Inspecting the earliest chronological events in the timeline reveals an immediate red flag:
          </p>

          <div className="relative bg-[#0a0a0f] border border-zinc-800/80 rounded-xl overflow-hidden">
            <div className="bg-[#14141e] px-4 py-2 flex justify-between items-center text-xs font-mono text-zinc-400 border-b border-zinc-800/80">
              <span>POWERSHELL • INSPECT EARLIEST ENTRIES</span>
              <CopyButton text={inspectCmd} />
            </div>
            <pre className="p-4 text-xs font-mono text-cyan-300 overflow-x-auto leading-relaxed">
              <code>{inspectCmd}</code>
            </pre>
          </div>

          <div className="bg-[#0e0e14] p-4 rounded-xl border border-zinc-800/70 font-mono text-xs space-y-1 text-zinc-300">
            <div className="text-zinc-500">// Earliest Timeline Records:</div>
            <div>Xxx Xxx 00 0000 00:00:00, 0, macb, V/V---------, 0, 0, 119417, "/$OrphanFiles"</div>
            <div className="text-amber-400 font-bold">Tue Jan 01 1985 17:00:00, 41, macb, r/rrw-r--r--, 0, 0, 4945, "/bin/bcab"</div>
            <div>Mon Oct 18 2021 17:54:17, 451, ma.., r/rrw-r--r--, 0, 0, 64994, "/usr/share/apk/keys/..."</div>
          </div>

          <p className="text-zinc-300 leading-relaxed text-sm">
            The target image runs a modern 2021–2025 Alpine Linux distribution. A file stamped to <strong>Tuesday, January 1, 1985</strong> is an undeniable <strong>temporal outlier</strong>. Furthermore, all four MACB timestamp flags (Modified, Accessed, Changed, Born) share the exact artificial second, proving deliberate timestomping on <code className="text-amber-300 font-mono">/bin/bcab</code> (Inode 4945).
          </p>
        </section>
        ...
        <section className="mb-12 space-y-6">
          ...
          <p className="text-zinc-300 leading-relaxed text-sm sm:text-base">
            Using <code className="text-cyan-300 font-mono">icat</code>, we carve the raw data blocks mapped to Inode 4945 directly:
          </p>
          ...
          <div className="bg-[#0e0e14] p-3 rounded-xl border border-zinc-800 font-mono text-xs text-zinc-300">
            ...
            <span className="text-cyan-300">NzFtMzExbjNfMHU3MTEzcl9oM3JfNDNhMmU3YWYK</span>
          </div>
          ...
          <div className="bg-[#0e0e14] p-3 rounded-xl border border-zinc-800 font-mono text-xs text-zinc-300">
            ...
            <span className="text-emerald-400 font-bold break-all block sm:inline">71m311n3_0u7113r_h3r_43a2e7af</span>
            <span className="text-zinc-500 block text-[11px] mt-1">(L33tspeak: &quot;timeline_outlier_her_43a2e7af&quot;)</span>
          </div>
