import Image from "next/image";
import Link from "next/link";

export default function HackTheBoxHub() {
  return (
    <div className="flex flex-col min-h-screen relative z-10 text-gray-200 selection:bg-emerald-500/30 selection:text-emerald-200">
      
      {/* Navigation */}
      <nav className="w-full p-6 flex justify-between items-center max-w-6xl mx-auto z-50 relative">
        <Link 
          href="/" 
          className="group flex items-center gap-2 text-zinc-400 hover:text-white transition-colors font-mono text-sm uppercase tracking-widest"
        >
          <span className="transform transition-transform group-hover:-translate-x-1">←</span> Back Home
        </Link>
      </nav>

      {/* Main Content */}
      <main className="flex-grow pt-10 pb-12">
        
        {/* Hub Header */}
        <div className="max-w-4xl mx-auto px-6 mb-16 text-center">
          <div className="mb-6 inline-block bg-[#111111]/80 backdrop-blur-md border border-emerald-500/40 px-4 py-1.5 rounded-full animate-glow-pulse">
            <span className="font-mono text-xs text-emerald-400 uppercase tracking-[0.3em]">
              HTB SHERLOCKS & LABS
              <span className="animate-blink inline-block w-1.5 h-3 bg-emerald-400 ml-2 align-middle"></span>
            </span>
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold text-white tracking-widest font-[family-name:var(--font-silkscreen)] mb-6 drop-shadow-[0_0_20px_rgba(16,185,129,0.5)]">
            HACK THE BOX
          </h1>
          <p className="text-zinc-400 text-lg md:text-xl font-sans max-w-2xl mx-auto leading-relaxed">
            Advanced incident response, DFIR scenarios, and enterprise intrusion case studies from HackTheBox.
          </p>
        </div>

        {/* Categories / Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-6 w-full">
          
          {/* VelvetThrone */}
          <Link href="/posts/velvet-throne" className="bg-[#111111]/60 backdrop-blur-md rounded-2xl flex flex-col shadow-lg border-2 border-zinc-800/60 transition-all duration-500 hover:scale-[1.02] hover:shadow-[0_10px_40px_rgba(16,185,129,0.2)] hover:border-emerald-500/50 group overflow-hidden">
            <div className="relative w-full h-48 overflow-hidden bg-black/50 border-b border-zinc-800/50">
              <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-[#111111] z-10 opacity-80 group-hover:opacity-40 transition-opacity duration-500"></div>
              <Image 
                src="/images/velvet_throne.jpg" 
                alt="VelvetThrone Sherlock Cover" 
                fill 
                className="object-cover transition-transform duration-700 group-hover:scale-105 brightness-[0.85] contrast-[1.2]" 
              />
            </div>
            
            <div className="p-6 flex flex-col flex-grow relative z-20 bg-transparent">
              <div className="flex justify-between items-center mb-4">
                <div className="bg-red-500/20 text-red-400 border border-red-500/30 px-2 py-0.5 rounded text-xs font-mono tracking-[0.2em] uppercase">
                  DIFFICULTY: HARD
                </div>
                <div className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]"></div>
              </div>
              <div className="mb-3 flex-grow">
                <h2 className="text-2xl font-bold mb-2 transition-colors duration-500 text-gray-200 group-hover:text-white leading-tight font-[family-name:var(--font-share-tech)] uppercase">
                  VelvetThrone
                </h2>
                <p className="text-gray-400 text-sm font-sans leading-relaxed">
                  Enterprise DFIR Investigation: MFA Fatigue, LSASS Dump, RC4 C2, and Covert DNS Exfiltration.
                </p>
              </div>
              <div className="flex items-center gap-2 mt-4 text-xs font-mono text-emerald-400/80 uppercase tracking-wider">
                <span>15 Forensic Tasks</span>
                <span>•</span>
                <span>4 Analysis Methods</span>
              </div>
            </div>
          </Link>

          {/* Placeholder for Next Sherlock / Lab */}
          <div className="bg-[#111111]/30 backdrop-blur-md rounded-2xl flex flex-col justify-center items-center shadow-lg border-2 border-dashed border-zinc-800/60 min-h-[300px] text-zinc-600 font-mono text-sm p-6 text-center">
            <span className="text-emerald-500/40 text-2xl mb-2">⚡</span>
            <span>+ NEXT SHERLOCK INCOMING</span>
          </div>

        </div>
      </main>
    </div>
  );
}
